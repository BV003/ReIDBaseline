...
from clustercontrast.trainers import  ClusterContrastTrainer_SDCL, Trainer
...
#保存 特征，标签，聚类数量
def write_features_and_clusters_to_file(features, pseudo_labels, num_clusters, file_name):
    with open(file_name, 'w') as f:
        ...
        for i, feature in enumerate(features):
            feature_str = ', '.join(f'{val:.4f}' for val in feature)
            label = pseudo_labels[i]
            f.write(f'Feature {i}: {feature_str}, Cluster label: {label}\n')
...
def main_worker_stage2(args,log_name):
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            cluster_ir = DBSCAN(eps=ir_eps, min_samples=1, metric='precomputed', n_jobs=-1)
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset_rgb.train), pseudo_labels_rgb)):
            if (label.item() != -1):
                ...
                #pseudo_labels_rgb[i] =num_cluster_rgb +outliers_rgb
                outliers_rgb+=1
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset_ir.train), pseudo_labels_ir)):
            if (label.item() != -1):
                ...
                pseudo_labeled_dataset_ir.append((fname,num_cluster_ir+outliers_ir,cid))#噪声点分类
                #pseudo_labels_ir[i] =num_cluster_ir +outliers_ir
                outliers_ir+=1

        print('==> Statistics for IR epoch {}: {} clusters outlier {} '.format(epoch, num_cluster_ir,outliers_ir))
